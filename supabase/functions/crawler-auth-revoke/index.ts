...
serve(async (req) => {
  ...
  try {
    ...
    if (!serviceBaseUrl) {
      return new Response(JSON.stringify({ success: false, error: "Crawler service disabled" }), {
        status: 503,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    const serviceToken = Deno.env.get("CRAWLER_SERVICE_TOKEN") || "";
    const endpoint = `${serviceBaseUrl.replace(/\/$/, "")}/internal/v1/auth/sessions/revoke`;
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: "Revoke session failed", detail: text.slice(0, 500) }), {
        status: 502,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
