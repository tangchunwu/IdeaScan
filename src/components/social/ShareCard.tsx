    ...
    <div className="space-y-4">
      ...
      >
        <div className="text-center mb-6">
          <div className="text-xs text-muted-foreground uppercase tracking-widest mb-2">综合得分</div>
          <h2 className="text-lg font-bold text-foreground line-clamp-2 leading-tight">{idea}</h2>
        ...
        <div className="flex flex-col items-center py-6">
          <div className={`text-6xl font-black bg-gradient-to-r ${verdictInfo.color} bg-clip-text text-transparent`}>{score}</div>
        ...
        {verdict && (
          <div className="text-center text-sm text-muted-foreground italic mb-6 px-4">"{verdict}"</div>
        )}

        <div className="space-y-2 mb-6">
          <div className="text-xs text-muted-foreground uppercase tracking-wider text-center">Top Scores</div>
          {topDimensions.map((d, i) => (
            <div key={i} className="flex justify-between items-center text-sm px-2 py-1 rounded bg-white/5">
              ...
              <span className={`font-bold ${d.score >= 70 ? 'text-green-500' : 'text-foreground'}`}>{d.score}</span>
            </div>
          ))}
        </div>

        <div className="text-center">
          <div className="text-[10px] text-muted-foreground/50">Powered by IdeaScan</div>
        </div>
      </div>
